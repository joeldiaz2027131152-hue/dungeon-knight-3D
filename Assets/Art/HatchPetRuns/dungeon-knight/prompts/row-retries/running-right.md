Create Codex pet row `running-right` for `dungeonknight`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Use the provided full-body knight exactly as the character identity: horned helmet, black steel armor, gold trim, blue torn cape, long silver sword, retro 2D fantasy sprite.. Preserve silhouette, face, palette, material, proportions, markings, and props.
