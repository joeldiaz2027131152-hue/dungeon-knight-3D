...
namespace DungeonKnight.Level
{
    public class WorldOneOneBootstrap : MonoBehaviour
    {
        ...
        private void Build()
        {
            ...
            SpriteRenderer knightVisual = CreateKnightVisual(player, CharacterSpriteFactory.KnightIdleA());
            SetLayerIfExists(player, "Player");
            player.AddComponent<PlayerInventory>();
            player.AddComponent<PlayerController2D>();
            player.AddComponent<PlayerInteraction>();
            player.AddComponent<CharacterFrameAnimator>().Configure(CharacterFrameAnimator.VisualKind.Knight, knightVisual);
        ...
        public static void BuildWorldOneThree()
        {
            ...
            CreatePlatform("World 1-3 Spire Ceiling Blocker", new Vector2(offset + towerCenterX, topY + 3.25f), new Vector2(towerWidth, 0.8f), new Color(0.12f, 0.115f, 0.17f));
            CreatePlatformTrim(new Vector2(offset + towerCenterX, topY + 2.81f), towerWidth);
        ...
        private static SpriteRenderer CreateKnightVisual(GameObject player, Sprite sprite)
        {
            if (player.TryGetComponent(out SpriteRenderer bodyRenderer))
            {
                bodyRenderer.enabled = false;
            }

            DisableLegacyKnightVisuals(player.transform, null);
        ...
        private static void DisableLegacyKnightVisuals(Transform player, Transform keep)
        {
            for (int i = 0; i < player.childCount; i++)
            {
                Transform child = player.GetChild(i);
                if (child == keep || !child.name.StartsWith("Knight Visual 2D")) continue;
                if (child.TryGetComponent(out SpriteRenderer duplicateRenderer)) duplicateRenderer.enabled = false;
